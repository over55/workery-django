...
class Command(BaseCommand):
    help = _('This hotfix should be run for issue `https://github.com/over55/workery-front/issues/214`.')

    def add_arguments(self, parser):
        """
        Run manually in console:
        python manage.py hotfix_28 "london"
        """
        ...
        for order in WorkOrder.objects.filter(
            ...
            try:
                with freeze_time(order.last_modified):
                    WorkOrderDeposit.objects.update_or_create(
                        order=order,
                        defaults={
                            'order': order,
                            'paid_at': order.invoice_date,
                            'amount': order.invoice_amount_due,
                            'deposit_method': WorkOrderDeposit.DEPOSIT_METHOD.CHEQUE,
                            'paid_to': WORK_ORDER_PAID_TO.ASSOCIATE,
                            'paid_for': WorkOrderDeposit.DEPOSIT_FOR.AMOUNT_DUE,
                        }
                    )
                    order.invoice_amount_due = 0
                    order.save()
                    self.stdout.write(
                        self.style.SUCCESS(_('Zeroed Order %s.') % str(order.id))
                    )
