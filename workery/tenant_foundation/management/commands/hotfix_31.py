...
class Command(BaseCommand):
    help = _('This hotfix should be run for issue `https://github.com/over55/workery-front/issues/263`.')

    def add_arguments(self, parser):
        """
        Run manually in console:
        python manage.py hotfix_31 "london"
        """
    ...
    def handle(self, *args, **options):
        ...
        for customer in Customer.objects.all().iterator(chunk_size=250):
            try:
                with freeze_time(customer.last_modified):
                    customer.save()
                    self.stdout.write(
                        self.style.SUCCESS(_('Updated customer %s.') % str(customer.id))
                    )
            except Exception as e:
                print(e)
        ...
        for job in WorkOrder.objects.all().iterator(chunk_size=250):
            try:
                with freeze_time(job.last_modified):
                    job.save()
                    self.stdout.write(
                        self.style.SUCCESS(_('Updated job %s.') % str(job.id))
                    )
            except Exception as e:
                print(e)

        for file in PrivateFileUpload.objects.all().iterator(chunk_size=250):
            try:
                with freeze_time(file.last_modified_at):
                    file.save()
                    self.stdout.write(
                        self.style.SUCCESS(_('Updated file %s.') % str(file.id))
                    )
            except Exception as e:
                print(e)

        self.stdout.write(
            self.style.SUCCESS(_('Successfully updated all unified search items.'))
        )
