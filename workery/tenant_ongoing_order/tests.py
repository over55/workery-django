...
class TestTenantOngoingOrderViews(TenantTestCase):
    """
    Console:
    python manage.py test tenant_ongoing_order.tests
    """
    ...
    def setUp(self):
        ...
        """
        super(TestTenantOngoingOrderViews, self).setUp()
    ...
    def tearDown(self):
        ...
        # Finish teardown.
        super(TestTenantOngoingOrderViews, self).tearDown()

    def test_ongoing_job_list_page(self):
        a_url = self.tenant.reverse(reverse_id='workery_tenant_ongoing_job_list')
        response = self.auth_c.get(a_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_ongoing_job_terminated_list_page(self):
        a_url = self.tenant.reverse(reverse_id='workery_tenant_ongoing_job_terminated_list')
        response = self.auth_c.get(a_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_ongoing_job_lite_retrieve_page(self):
        ongoing_work = OngoingWorkOrder.objects.get()
        a_url = self.tenant.reverse(
            reverse_id='workery_tenant_ongoing_job_lite_retrieve',
            reverse_args=[
                'summary',
                int(ongoing_work.id)
            ]
    ...
    def test_ongoing_job_full_retrieve_page(self):
        ongoing_work = OngoingWorkOrder.objects.get()
        a_url = self.tenant.reverse(
            reverse_id='workery_tenant_ongoing_job_full_retrieve',
            reverse_args=[
                'summary',
                int(ongoing_work.id)
            ]
    ...
    def test_ongoing_job_retrieve_for_comments_page(self):
        ongoing_work = OngoingWorkOrder.objects.get()
        a_url = self.tenant.reverse(
            reverse_id='workery_tenant_ongoing_job_retrieve_for_comments',
            reverse_args=[
                'summary',
                int(ongoing_work.id)
            ]
    ...
    def test_ongoing_job_update_page(self):
        ongoing_work = OngoingWorkOrder.objects.get()
        a_url = self.tenant.reverse(
            reverse_id='workery_tenant_ongoing_job_update',
            reverse_args=[
                'summary',
                int(ongoing_work.id)
            ]
