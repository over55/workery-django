...
class CanListCreateOrderServiceFeePermission(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if "GET" in request.method:
            return has_permission('can_get_order_service_fees', request.user, request.user.groups.all())

        # --- CREATE ---
        if "POST" in request.method:
            return has_permission('can_post_order_service_fee', request.user, request.user.groups.all())
...
class CanRetrieveUpdateDestroyOrderServiceFeePermission(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if "GET" in request.method:
            ...
            # PERMISSION BASED
            return has_permission('can_get_order_service_fee', request.user, request.user.groups.all())
        ...
        if "PUT" in request.method:
            ...
            # PERMISSION BASED
            return has_permission('can_put_order_service_fee', request.user, request.user.groups.all())

        # --- DELETE ---
        if "DELETE" in request.method:
            # PERMISSION BASED
            return has_permission('can_delete_order_service_fee', request.user, request.user.groups.all())
